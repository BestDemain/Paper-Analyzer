...
class BaseTranslator:
    ...
    def translate(self, text: str, ignore_cache: bool = False) -> str:
        ...
        if self.isllm:
            translation = self.get_completion(text, mode = "translate")
            prompt_reflection = self.prompt_reflect(text, translation)
            reflection = self.get_completion(prompt_reflection, mode = "reflect")
            prompt_improvement = self.prompt_improve(text, translation, reflection)
            translation = self.get_completion(prompt_improvement, mode = "improve")
        else:
            translation = self.get_completion(text, mode = "translate")
    ...
    def get_completion(self, text: str | list, mode : str) -> str:
        """
    ...
    def prompt_reflect(self, text: str, translation: str) -> list[dict[str, str]]:
        return [
            {
                "role": "user",
                "content": (
                    f"""Your task is to carefully read a source text and a translation from {self.lang_in} to {self.lang_out}, and then give constructive criticism and helpful suggestions to improve the translation. \
                    ...
                    (ii) fluency (by applying {self.lang_out} grammar, spelling and punctuation rules, and ensuring there are no unnecessary repetitions),\n\
                    (iii) style (by ensuring the translations reflect the style of the source text and take into account any cultural context),\n\
                    (iv) terminology (by ensuring terminology use is consistent and reflects the source text domain; and by only ensuring you use equivalent idioms {self.lang_out}).\n\
    ...
    def prompt_improve(self, text: str, translation: str, reflection: str) -> list[dict[str, str]]:
        return [
            {
                "role": "user",
                "content": (
                    f"""Your task is to carefully read, then edit, a translation from {self.lang_in} to {self.lang_out}, taking into
...
class GoogleTranslator(BaseTranslator):
    ...
    def get_completion(self, text, mode = "translate"):
...
class OllamaTranslator(BaseTranslator):
    ...
    def get_completion(self, text: str) -> str:
        ...
        response = self.client.chat(
            model=self.model,
            messages=self.prompt_translate(text, self.prompt_template),
    ...
    def get_completion(self, text):
        ...
        for model in self.model.split(";"):
            try:
                ...
                xf_prompt = self.prompt_translate(text, self.prompttext)
                xf_prompt = [
                    {
                        "role": "user",
...
class OpenAITranslator(BaseTranslator):
    ...
    def __init__(
        ...
        self.prompttext = prompt
        self.add_cache_impact_parameters("temperature", self.options["temperature"])
        self.add_cache_impact_parameters("prompt", self.prompt_translate("", self.prompttext))
    ...
    def get_completion(self, text, mode) -> str:
        if mode == "translate":
            messages = self.prompt_translate(text, self.prompttext)
        else:
            messages = text
